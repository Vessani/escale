...
export default function NovaViagemPage() {
  ...
  const [mostrarFormularioAvulso, setMostrarFormularioAvulso] = useState(false)
  const [revisandoLote, setRevisandoLote] = useState<ViagemParaConfirmar[] | null>(null)
  ...
  const handleConcluidoLote = (resultado: ResultadoImportacaoLote) => {
    setRevisandoLote(null)
  ...
  const onSubmit: SubmitHandler<NovaViagemFormValues> = async (dados) => {
    ...
    try {
      const resposta = await criarViagemAvulsa(dados)
      ...
      {revisandoLote ? (
        ...
        <>
          ...
          {!mostrarFormularioAvulso ? (
            <Button type="button" variant="outline" onClick={() => setMostrarFormularioAvulso(true)}>
